...
//üstteki alana istediğiniz oyunları girebilirsiniz önerim dokunmamanız.
...
class Bot {
    ...
    online() {
        ...
        this.bot.on('ready', () => {
            console.log(colors.green(`[BILGI]`) + `|` + (colors.brightBlue `Aktifleştirildi!`) + (colors.brightRed `%s`) + ` |` + (colors.brightBlue ` ID: `) + (colors.magenta `%s`) + ` ve` + (colors.yellow ` "%s"`), this.bot.user.tag, this.bot.user.id, new Date().toLocaleTimeString());
        ...
        this.bot.on('ready', () => {
            this.bot.user.setPresence({
                game: {
                    ...
                    type: "PLAYING" //burayı WATCHING yapabilirsiniz tokenlerimizin durumunu belirler.
                }
            });
        });
...
process.title = `Company Token Aktif Etme Sistemi Toplam Token: ${tokens.length}`;
console.log(colors.green(`Company Token Aktif Edici`) + (colors.magenta(`Epic `)) + (colors.cyan(`[Company]`)) + (colors.red(`Toplam Token: ${tokens.length}`)));

console.log(colors.inverse(`[BILGI] - [Token Aktif Edici]`));
